...
impl<T> DecisionDiagram for All<T>
where
    T: Eq + PartialEq + Hash + Clone,
{
    type State = T;
...
where
    T: Eq + PartialEq + Hash + Clone,
{
    pub fn new(cutset_type: CutsetType) -> Self {
        Self {
            root_pa: vec![],
            nodes: vec![],
            edges: vec![],
                ...
                for node_id in next_v.drain(..) {
                    ...
                    while let Some(edge_id) = inbound {
                        ...
                        let lp_from_bot_using_edge =
                            self.nodes[node_id.0].value_bot.saturating_add(edge.cost);
